// Simple JSON store (data/store.json) + game utilities.
// Features:
// - Auto-creates rounds with random duration and pair (ATOM/BTC/ETH/OSMO)
// - Value & Direction predictions (1 per player per round per mode)
// - Settlement -> score calculation + event log
// - Leaderboard, event feed, vacuum (prune)
// - Bet tickets (testnet): attach stake tx; payout helpers (optional)

import fs from "node:fs/promises";
import path from "node:path";
import { getAtomPriceUSD, getPriceUSD } from "./price.js";
import { sendFromTreasury } from "./pay.js";
import { PAYOUT_MULTIPLIER, HOUSE_FEE_BP } from "./config.js";
...
export async function read() {
  ...
  try {
    ...
    db.predictions_dir ||= [];
    db.events ||= [];
    db.tickets ||= []; // bet tickets
    db.bets ||= [];    // reserved for advanced betting (optional)
    db.meta ||= {};
    return db;
  } catch (e) {
    if (e.code === "ENOENT") {
...
// ---------------- Rounds ----------------
const ROUND_MIN_MIN = 2;
const ROUND_MIN_MAX = 10;

const PAIR_LIST = ["ATOM", "BTC", "ETH", "OSMO"];
const PAIR_TO_POOL = { ATOM: "1", BTC: "2", ETH: "3", OSMO: "4" };
const POOL_TO_PAIR = { "1": "ATOM", "2": "BTC", "3": "ETH", "4": "OSMO" };
...
export async function addPredictionDir(playerId, roundId, direction) {
  ...
  if (!["UP", "DOWN"].includes(dir)) throw new Error("Invalid direction");
...
export async function createBetTicket({ roundId, playerId, fromAddr, amountUosmo, txhash }) {
  ...
  // de-dupe by txhash
  const exist = (db.tickets || []).find((t) => t.txhash === txhash);
...
async function settleRound(roundId) {
  ...
  let final = NaN;
  try {
    const out = await getPriceUSD(POOL_TO_PAIR[String(r.pool_id)] || r.pair || "ATOM");
  ...
  let went = "FLAT";
  if (Number.isFinite(start) && Number.isFinite(final)) {
    went = final === start ? "FLAT" : final > start ? "UP" : "DOWN";
  }
  r.direction = went;
  ...
  for (const pv of vals) {
    if (Number.isFinite(final) && final > 0) {
      const errPct = Math.abs(Number(pv.price_pred) - final) / final * 1000;
      pv.points = Math.max(0, Math.round(100 - errPct));
    } else {
      pv.points = 0;
    }
  }
  ...
  for (const pd of dirs) {
    pd.points = went !== "FLAT" && pd.direction === went ? 50 : 0;
  }

  pushEvent(db, {
    type: "Settled",
    text: `Settled ${r.id.slice(0, 6)} • pair ${r.pair} • final $${Number(final).toFixed(4)} • went ${went}`,
    rid: r.id,
  });

  await write(db);
  await payoutWinners(r); // no-op unless you populate db.bets
...
async function payoutWinners(round) {
  ...
  const bets = (db.bets || []).filter((b) => b.round_id === round.id && b.status === "LOCKED");
  ...
  for (const b of bets) {
    ...
    try {
      const hash = await sendFromTreasury(b.from, net, `Payout ${String(round.id).slice(0, 6)}`);
      b.status = "PAID";
      b.payout_uosmo = net;
      b.payout_txhash = hash || null;
      pushEvent(db, {
        type: "Payout",
        text: `Paid ${net} uosmo to ${b.from.slice(0, 8)}… (${round.id.slice(0, 6)})`,
        rid: round.id,
      ...
      pushEvent(db, {
        type: "PayoutFail",
        text: `Pending payout ${net} uosmo to ${b.from.slice(0, 8)}… (retry)`,
        rid: round.id,
